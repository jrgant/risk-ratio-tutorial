...
## Calculate the standard error for the risk ratio
...
## Calculate the standard error for the risk difference
...
# Direct estimation of a risk difference using linear probability models

When we have a binary outcome, as we do in our example (death), we can estimate the risk difference directly using linear probability models. We can fit a generalized linear model using a binomial distribution and an identity link, in which case the outcome distribution may be correctly specified, and we do not have to adjust the standard error estimates used to calculate confidence limits. 

Moving forward, we will assume that the binary outcomes discussed in this document imply binomially distributed outcomes with respect to the conditional means modeled in GLMs. Therefore, we will refer to the binomial distribution as being the "correct specification", but we note here that other distributions are possible (_e.g._, probit).

Assuming that we have correctly specified the outcome distribution as binomial, we can fit a gen-
eralized linear model using a binomial distribution and an identity link without needing to adjust
the standard error estimates used to calculate confidence limits.

In some cases, we may wish to use alternative models. The OLS model and the GLM with a Gaussian distribution are equivalent. Both of these models, plus the GLM using a Poisson distribution, require use of a "robust" sandwich estimator to estimate an unbiased standard error for the risk difference. This sandwich estimator accounts for misspecification of the probability distribution, _i.e._, the fact that we are fitting models that assume Gaussian or Poisson distributions to an outcome that follows a binomial distribution. (See Section \ref{sec-rr-sandwich} for further explication in the context of the risk ratio).

## Calculate a risk difference by fitting a generalized linear model, specifying an identity link and binomial probability distribution

Here, we provide code to estimate a risk difference using a **GLM with an identity link and a binomial probability distribution**. Given that in our example the outcome death is a binary variable and has a binomial distribution, the model is correctly specified; hence, no correction is needed for the standard error:

```{r rd-direct-glm-lin-bin}
glm_lin_bin_fit <- glm(death ~ weakheart,
                       data = nhefs,
                       family = binomial(link = "identity"))

# extract the coefficient for weakheart
glm_lin_bin_rd <- unname(coef(glm_lin_bin_fit)[2])

# note we use the standard variance-covariance matrix extractor
# no need for sandwich estimator under the correct probability distribution
glm_lin_bin_robse <- sqrt(diag(vcov(glm_lin_bin_fit))[2])
glm_lin_bin_robse <- unname(glm_lin_bin_robse)

c(RD = glm_lin_bin_rd,
  SE = glm_lin_bin_robse,
  `95% CL, Lower` = glm_lin_bin_rd - qnorm(0.975) * glm_lin_bin_robse,
  `95% CL, Upper` = glm_lin_bin_rd + qnorm(0.975) * glm_lin_bin_robse)
```


## Calculate a risk difference by fitting a linear model using ordinary least squares estimation
Here, we provide code to estimate a risk difference using **OLS**. Because this model is incorrectly specified (assumes a Gaussian distribution when we are working with a binomially distributed outcome), we must estimate "robust" standard errors:

```{r rd-direct-lm}
lm_fit <- lm(death ~ weakheart, data = nhefs)

# extract the coefficient for weakheart
lm_fit_rd <- unname(coef(lm_fit)[2])

# extract the variance for the weakheart coeffient and
# take the square root to get the standard error
lm_fit_robse <- sqrt(diag(vcovHC(lm_fit, type = "HC0"))[2])
lm_fit_robse <- unname(lm_fit_robse)

c(RD = lm_fit_rd,
  RobustSE = lm_fit_robse,
  `95% CL, Lower` = lm_fit_rd - qnorm(0.975) * lm_fit_robse,
  `95% CL, Upper` = lm_fit_rd + qnorm(0.975) * lm_fit_robse)
```

## Calculate a risk difference by fitting a generalized linear model, specifying an identity link and a Gaussian probability distribution
Here, we provide code to estimate a risk difference using a **GLM with an identity link and a Gaussian probability distribution**. Because this model is incorrectly specified (assumes a Gaussian distribution when we are working with a binomially distributed outcome), we must estimate "robust" standard errors:

```{r rd-direct-glm-lin-gaussian}
glm_lin_gauss_fit <- glm(death ~ weakheart,
                     data = nhefs,
                     family = gaussian(link = "identity"))

glm_lin_gauss_rd <- unname(coef(glm_lin_gauss_fit)[2])
glm_lin_gauss_robse <- sqrt(diag(vcovHC(glm_lin_gauss_fit, type = "HC0"))[2])
glm_lin_gauss_robse <- unname(glm_lin_gauss_robse)

c(RD = glm_lin_gauss_rd,
  RobustSE = glm_lin_gauss_robse,
  `95% CL, Lower` = glm_lin_gauss_rd - qnorm(0.975) * glm_lin_gauss_robse,
  `95% CL, Upper` = glm_lin_gauss_rd + qnorm(0.975) * glm_lin_gauss_robse)
```

## Calculate a risk difference by fitting a generalized linear model, specifying an identity link and a Poisson probability distribution
Here, we provide code to estimate a risk difference using a **GLM with an identity link and a Poisson probability distribution**. Because this model is incorrectly specified (assumes a Poisson distribution when we are working with a binomially distributed outcome), we must estimate "robust" standard errors:

```{r rd-direct-glm-lin-pois}
glm_lin_pois_fit <- glm(death ~ weakheart,
                        data = nhefs,
                        family = poisson(link = "identity"))

glm_lin_pois_rd <- unname(coef(glm_lin_pois_fit)[2])
glm_lin_pois_robse <- sqrt(diag(vcovHC(glm_lin_pois_fit, type = "HC0"))[2])
glm_lin_pois_robse <- unname(glm_lin_pois_robse)

c(RD = glm_lin_pois_rd,
  RobustSE = glm_lin_pois_robse,
  `95% CL, Lower` = glm_lin_pois_rd - qnorm(0.975) * glm_lin_pois_robse,
  `95% CL, Upper` = glm_lin_pois_rd + qnorm(0.975) * glm_lin_pois_robse)
```


# Direct estimation of a risk ratio using a log-binomial regression model

## Fit a log-binomial regression for the relationship between heart medication and mortality

We can fit a log-binomial regression model in R by using the `glm()` function from the `stats` package by specifying a binomial distribution and a (natural) log link, which is passed to the `family` argument of `glm()` as shown below. We extract $\beta_1$ and use `tidy()` from the `broom` package to exponentiate the coefficient, yielding a risk ratio, and include the 95% confidence interval in our model output.
...
## Fit a Poisson regression for the relationship between heart medication and mortality

We can fit a Poisson regression model in R by using the `glm()` function from the `stats` package by specifying a Poisson distribution and a (natural) log link, which is passed to the `family` argument of `glm()` as shown below. We extract $\beta_1$ and use `tidy()` from the `broom` package to expontentiate the coefficient, yielding a risk ratio, and include the 95% confidence interval in our model output.
...
# Return second row to focus on the weakheart variable
model_output <- as.data.frame(model_output)
knitr::kable(model_output[2, ], digits = 4)
```

## Estimate 95% confidence limits for the risk ratio via non-parametric bootstrapping.
...
We can fit a logistic regression model in R by using the `glm()` function from the `stats` package by specifying a binomial distribution, as shown below. The `binomial()` function uses a logit link by default. 

## Fit a logistic regression for the relationship between heart medication and mortality
...
## Predict the expected probability of death for each weakheart group.
...
## Estimate 95% confidence limits for the risk ratio via non-parametric boostrapping.
...
## Write a function to evaluate repeatedly (*i.e.*, within each bootstrap replicate).
...
## Run analysis within each bootstrap replicate.
