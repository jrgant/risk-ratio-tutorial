...
We can estimate the risk difference directly using a variety of linear probability models for binary outcomes. Some common choices include the following:

1. ordinary least squares (OLS)
2. maximum likelihood estimation via generalized linear models, all using an identity link but different probability distributions:
   a) binomial (correctly specified) 
   b) Poisson (incorrectly specified)
   c) Gaussian (incorrectly specified)
   
The OLS model and the GLM with a Gaussian distribution are equivalent. Both of these models and the Poisson GLM require the use of a "robust" sandwich estimator to estimate the standard error for the risk difference in order to account for misspecification of the probability distribution (see Section \ref{sec-rr-sandwich} for further explication in the context of the risk ratio). The GLM using a binomial probability distribution requires no such correction.
...
c(RD = coef(lm_fit)[2],
  RobustSE = sqrt(diag(vcovHC(lm_fit, type = "HC0"))[2]))
```
...
c(RD = coef(glm_lin_bin_fit)[2],
  SE = sqrt(diag(vcov(glm_lin_bin_fit))[2]))
```
...
c(RD = coef(glm_lin_gauss_fit)[2],
  RobustSE = sqrt(diag(vcovHC(glm_lin_gauss_fit, type = "HC0"))[2]))
```
...
c(RD = coef(glm_lin_pois_fit)[2],
  RobustSE = sqrt(diag(vcovHC(glm_lin_pois_fit, type = "HC0"))[2]))
```
