...
pacman::p_load(readr, sandwich, finalfit, boot)
```
# NHEFS data
```{r}
...
write_csv(model_output, 'Modified Poisson Model.csv')
```
...
bootCI = function(data, formula, i){
  data = nhefs[i,]
...
boot_estimate = boot(bootCI, nhefs, R = 1000)
plot(boot_estimate)
